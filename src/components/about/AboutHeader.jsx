...
export function AboutHeader({ volunteerCount = 14 }) {
  return (
    <section className="bg-white rounded-xl border border-slate-300 overflow-hidden shadow-2xs">
      {/* Cover Banner Image */}
      ...
      <div className="px-4 sm:px-5 pb-4 sm:pb-5 pt-0">
        <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-3.5 -mt-12 sm:-mt-14 mb-3.5">
          ...
          <div className="flex flex-wrap items-center gap-2 pt-2 sm:pt-0">
            <Link href="/program">
              <Button className="h-9 sm:h-10 px-4 sm:px-5 text-xs sm:text-sm font-semibold text-white bg-gradient-to-b from-slate-800 via-slate-900 to-slate-950 hover:from-slate-700 hover:via-slate-800 hover:to-slate-950 border-t border-t-slate-600 border-x border-x-slate-700 border-b-2 border-b-slate-950 shadow-[inset_0_1px_0_rgba(255,255,255,0.25),0_3px_6px_rgba(0,0,0,0.35)] active:translate-y-0.5 active:shadow-[inset_0_1px_0_rgba(255,255,255,0.1)] transition-all cursor-pointer">
                Salurkan Donasi
              </Button>
            </Link>
            ...
            <ShareDropdown
              ...
              align="end"
              variant="icon"
              className="h-9 sm:h-10 w-9 sm:w-10 rounded-lg border border-slate-300 bg-white hover:bg-slate-100 text-slate-700 flex items-center justify-center shadow-2xs shrink-0"
            />
          </div>
        </div>
