...
export function AboutHeader({
	...
	const [internalCopied, setInternalCopied] =
		useState(false);
	const isCopied =
		copiedProfile !== undefined
			? copiedProfile
			: internalCopied;
	const leadershipCount =
		ORG_PROFILE.leadership
			?.length ||
		4;
	...
	const handleShare = () => {
		...
		if (
			...
			toast.success(
				"Tautan profil yayasan berhasil disalin.",
			);
			setTimeout(
				() =>
					setInternalCopied(
			...
			<div className="px-5 sm:px-8 pb-5 sm:pb-6 pt-0">
				...
				<div className="space-y-2.5">
					...
					<div className="flex flex-wrap items-center gap-y-1.5 gap-x-3 text-xs sm:text-sm text-slate-700 pt-2 border-t border-slate-200">
						...
						<span
							className="text-slate-300 select-none hidden sm:inline"
							aria-hidden="true"
						>
							|
						</span>
