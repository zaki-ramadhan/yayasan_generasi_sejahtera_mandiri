        ...
        {item.isCustom && (
          <div className="animate-in fade-in slide-in-from-top-1 duration-150">
            <Input
              type="text"
              autoFocus
              placeholder="Masukkan nominal kustom (Min. Rp 10.000)"
              ...
              onChange={(e) => {
                ...
                onChange(
                  item.id,
                  "customAmount",
                  raw ? formatRupiah(parseInt(raw, 10)) : ""
                );
                onChange(item.id, "amount", raw ? parseInt(raw, 10) : 0);
              }}
              className="h-10 text-sm text-slate-900 border-slate-300 mt-2"
            />
          </div>
        )}
      </div>
